...
struct NewOrder : Head {
    ...
    U16 qty;
    U8 sz;
    BlockRef<RptGrp> grp() {return BlockRef<RptGrp>(begin()+fixed_size(), sz);}
    BlockRef<RptGrp> grp() const {return BlockRef<RptGrp>(cbegin()+fixed_size(), sz);}
    uint16_t bitmem : 28;
    std::string_view sss() {return std::string_view(bitmem.end(), sz.raw_value()-2);}
    std::string_view sss() const {return std::string_view(bitmem.end(), sz.raw_value()-2);}
    FloatingRef<U16> plen() {return FloatingRef<U16>(sss().end());}
    FloatingRef<U16> plen() const {return FloatingRef<U16>(sss().cend());}
    //this member should be only varlen member and the last one
    auto appendage() {return OptionalByLengthRef<Entry, AppendageType, decltype(plen)&>(begin()+fixed_size(), plen);}
    auto appendage() const {return OptionalByLengthRef<Entry, AppendageType, const decltype(plen)&>(cbegin()+fixed_size(), plen);}
    FloatingRef<U8> pmap() {return FloatingRef<U8>(appendage().end());}
    FloatingRef<U8> pmap() const {return FloatingRef<U8>(appendage().cend());}
    ...
    size_t size() {return firm_arr().end()-begin();}
    size_t size() const {return firm_arr().cend()-cbegin();}
...
inline std::ostream& operator<<(std::ostream& os, const NewOrder& msg) {
    ...
    os << "price=" << msg.price << ";"
       ...
       << "sss=" << const_cast<NewOrder&>(msg).sss() << ";"
       << "plen=" << const_cast<NewOrder&>(msg).plen() << ";"
       << "appendage=" << const_cast<NewOrder&>(msg).appendage() << ";"
       << "pmap=" << const_cast<NewOrder&>(msg).pmap() << ";"
       << "firm_arr=" << const_cast<NewOrder&>(msg).firm_arr() << ";"
       << "}";
    return os;
}
