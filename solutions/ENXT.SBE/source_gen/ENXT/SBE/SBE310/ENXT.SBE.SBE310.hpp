...
using uint64_t = LittleEndian<uint64_t, 0, 18446744073709551614, 18446744073709551615>;
using int32_t = LittleEndian<int32_t, -2147483647, 2147483647, -2147483648>;
using int64_t = LittleEndian<int64_t, -9223372036854775807, 9223372036854775807, -9223372036854775808>;
using time_t = LittleEndian<uint64_t, 0, 9223372036854775807, 0>;
using char1 = char;
...
EB_BITFIELD(uint8_t, ExecutionInstruction_set,
  ((STPRestingOrder,0,1))
...
EB_BITFIELD(uint8_t, DarkExecutionInstruction_set,
  ((DarkIndicator,0,1))
...
EB_BITFIELD(uint8_t, AckQualifiers_set,
  ((DarkIndicator,0,1))
  ((QueueIndicator,1,1))
...
EB_BITFIELD(uint8_t, MiFIDIndicators_set,
  ((DEAIndicator,0,1))
...
EB_BITFIELD(uint16_t, OpenClose_set,
...
EB_BITFIELD(uint8_t, TradeQualifier_set,
  ((UncrossingTrade,0,1))
  ((FirstTradePrice,1,1))
  ((PassiveOrder,2,1))
  ((AggressiveOrder,3,1))
...
EB_BITFIELD(uint8_t, TradingSessionValidity_set,
  ((Session1,0,1))
  ((Session2,1,1))
  ((Session3,2,1))
  ((Session4,3,1))
...
EB_BITFIELD(uint8_t, WaiverIndicator_set,
  ((LRGS,0,1))
  ((RFPT,1,1))
  ((NLIQ,2,1))
  ((OILQ,3,1))
...
EB_BITFIELD(uint8_t, MMPExecutionType_set,
  ((Notification,0,1))
  ((Acknowledgement,1,1))
  ((Pull,2,1))
);
...
EB_BITFIELD(uint16_t, TargetCounterparties_set,
