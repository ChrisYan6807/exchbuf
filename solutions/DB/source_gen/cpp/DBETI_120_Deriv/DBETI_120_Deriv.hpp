...
EB_ENUM(BasketAnonymity, uint8_t, 
  ((No, 0))
  ((Yes, 1))
);
...
EB_ENUM(CrossPrioritization, uint8_t, 
  ((Buy_side, 1))
  ((Sell_side, 2))
);
using CrossRequestID = LittleEndian<int32_t, -2147483647, 2147483647, 0x80000000>;
EB_ENUM(CrossRequestType, uint8_t, 
  ((Cross, 1))
  ((Clip, 2))
);
EB_ENUM(CrossType, uint8_t, 
  ((Cross_IOC, 2))
  ((Cross_one_side, 3))
);
...
EB_ENUM(ImpliedCheckPriceIndicator, uint8_t, 
  ((No, 0))
  ((Yes, 1))
);
...
EB_ENUM(RiskLimitResult, uint16_t, 
