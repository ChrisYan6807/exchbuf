...
class LMEProtocol {
public:
    LMEProtocol(boost::asio::streambuf& rcv_buf, boost::asio::streambuf& send_buf, std::function<void()> send_cb, std::function<void(const TagValueMsg&, const std::string&)> rcvd_cb)
...
bool  LMEProtocol::on_raw_data_received() {
    ...
    while(true) {
        LME::MsgHeader* header = std::static_cast<LME::MsgHeader*>(boost::asio::buffers_begin(rcv_buf_.data()));
        if(header->msgLength.rawValue() >= rcv_buf_.size()) {
            switch(header->msgType.value()) {
                case LME::MsgType::Logon:
                    auto logon = std::static_cast<LME::Logon*>(header);
                    handle_logon(*logon);
                    break;
                case LME::MsgType::Logout:
                    auto logout = std::static_cast<LME::Logout*>(header);
                    handle_logout(*logout);
                    break;
                case LME::MsgType::ExecutionReport:
                    auto report = std::static_cast<LME::ExecutionReport*>(header);
                    handle_report(*report);
                    break;
                case LME::MsgType::Heartbeat:
                    auto heartbeat = std::static_cast<LME::Heartbeat*>(header);
            ...
            rcv_buf_.consume(header->msgLength.rawValue());
            if(rcv_buf_.size() < header_len) {
                break;
            }
        } else {
...
std::pair<bool, std::string>  LMEProtocol::send_msg(TagValueMsg& msg) {
    if(msg.has_tag(35)) {
...
void LMEProtocol::send_request(LME::MsgHeader* header) {
    ...
    std::ostream os(&send_buf_);
    os << std::string_view(header->begin(), header->msgLength.rawValue());
...
std::string LMEProtocol::send_new_order_single(TagValueMsg& order) {
...
void LMEProtocol::handle_logon(LME::Logon& msg) {
...
void LMEProtocol::handle_logout(const LME::Logout& msg) {
    //todo terminate session
}

void LMEProtocol::handle_heartbeat(const LME::Heartbeat& msg) {
    send_heartbeat();
}

void LMEProtocol::handle_report(LME::ExecutionReport& msg) {
    TagValueMsg tv_msg;
